...
(function (factory, window) {
    ...
    var MiniMap = L.Control.extend({
        ...
        fillViewport: function() {
            ...
            if(map.origSize){
              var w = map.origSize.w;
              var h = map.origSize.h;
              useZoom = 2
            }
            else{
              w = map.getBounds().getEast()  - map.getBounds().getWest();
              h = map.getBounds().getNorth() - map.getBounds().getSouth();
              map.origSize = {w: w, h: h};
            }
            var sw = this._map.options.crs.pointToLatLng(L.point(0, h), useZoom);
            var ne = this._map.options.crs.pointToLatLng(L.point(w, 0), useZoom);
            var bounds = L.latLngBounds(sw, ne);

            map.fitBounds(bounds, {padding: [0,0,0,0]});
        },
        ...
        onAdd: function (map) {
            ...
            this._innerContainer.style.width  = this.options.width() + 'px';
            this._innerContainer.style.height = 'calc(' + this.options.height() + 'px' + ' - ' + this.options.toolbarHeight + ')';
            //this._container.style.width = this.options.width + 'px';
            //this._container.style.height = this.options.height + 'px';
            ...
            this._miniMap.whenReady(L.Util.bind(function () {
                ...
                require(['util_resize'], function(resizeUtils){

                  resizeUtils.addDebouncedFunction('refresh-leaflet-mini-map', 'refreshMiniMap', 100);
        ...
        _restore: function(large) {
            ...
            if (this.options.toggleDisplay) {
                ...
                $(this._container).find('.mini-map-ctrls').removeClass('hidden');
